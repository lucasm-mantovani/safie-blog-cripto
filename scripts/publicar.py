...
def gerar_imagem_capa(artigo: dict, config_blog: dict) -> tuple:
    ...
    cor_dest  = iv.get("cor_destaque_imagem", "#4a9eff")
    cor_bco   = "#ffffff"
...
def atualizar_indice(artigo: dict):
    ...
    # Remove entrada anterior com mesmo slug (reescrita)
    ...
    # Manter os últimos 200 artigos no índice
    indice = indice[:200]
...
def atualizar_pagina_tema(tema_slug: str, indice: list, config_temas: dict):
    ...
    # Links de todos os temas para a sidebar
    todos_temas_links = ""
    for t in config_temas.get("temas", []):
        todos_temas_links += (
...
def atualizar_sitemap(artigo: dict, config_blog: dict):
    ...
    # Insere antes do comentário de artigos ou antes de </urlset>
...
def git_commit_push(artigo: dict):
    ...
    # Configura remote com token se necessário
...
def main(sem_git: bool = False):
    ...
    # 3. Recarregar índice atualizado
    indice = ler_json(INDICE_JSON, [])

    # 4. Atualizar home
    atualizar_home(indice, config_blog)

    # 5. Atualizar página do tema
